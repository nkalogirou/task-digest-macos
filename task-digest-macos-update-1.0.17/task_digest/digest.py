...
.priority-control { position: relative; flex: 0 0 auto; min-width: 92px; color: var(--priority-color); background: color-mix(in srgb,var(--priority-color) 10%,var(--surface-muted)); border-color: color-mix(in srgb,var(--priority-color) 28%,var(--border)); transition: border-color .16s ease, background .16s ease, opacity .16s ease; }
.priority-control::after { content: "⌄"; position: absolute; right: 8px; top: 50%; transform: translateY(-55%); color: currentColor; font-size: 11px; pointer-events: none; }
...
.priority-control select { width: 100%; min-width: 0; appearance: none; -webkit-appearance: none; border: 0; outline: 0; color: currentColor; background: transparent; padding: 5px 24px 5px 9px; font: inherit; font-size: 10px; font-weight: 720; cursor: pointer; }
.priority-control select:focus-visible { box-shadow: 0 0 0 3px color-mix(in srgb,var(--priority-color) 24%,transparent); border-radius: 999px; }
.priority-control option { color: CanvasText; background: Canvas; }
.priority-control .priority-manual-dot { position: absolute; left: 5px; top: 4px; width: 4px; height: 4px; border-radius: 50%; background: currentColor; pointer-events: none; }
...
def _priority_control(item: TaskItem) -> str:
    """Render an inline priority selector when dashboard actions are available."""
    if not _ACTION_TOKEN or not _DASHBOARD_URL:
        return f'<span class="priority-pill tone-{item.priority}">{escape(item.priority.title())}</span>'

    selected = item.manual_priority or ""
    automatic_label = f"Automatic · {item.priority.title()}"
    ...
    return (
        f'<form method="post" action="{escape(_DASHBOARD_URL + "/action", quote=True)}" '
        f'class="priority-control tone-{item.priority}{manual_class}" data-priority-form draggable="false">'
        ...
        '<input type="hidden" name="action" value="set_priority">'
        f'{manual_dot}<select name="priority" aria-label="{escape(label, quote=True)}" '
